...
final class PuzzleGameManager {

    static let shared = PuzzleGameManager()
    ...
    private func completionKey(for date: Date) -> String {
        ...
        return "puzzle_game_\(c.year!)_\(c.month!)_\(c.day!)_completed"
    }

    private func timeKey(for date: Date) -> String {
        let c = calendar.dateComponents([.year, .month, .day], from: date)
        return "puzzle_game_\(c.year!)_\(c.month!)_\(c.day!)_time"
    }
}
