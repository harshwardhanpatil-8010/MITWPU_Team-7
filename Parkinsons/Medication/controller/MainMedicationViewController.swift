...
final class MainMedicationViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // 1. App Active Observer (Block-based)
        ...
        // 2. Custom "MedicationLogged" Observer (Updated to Block-based)
        medicationLoggedObserver = NotificationCenter.default.addObserver(
            forName: NSNotification.Name("MedicationLogged"),
            object: nil,
            queue: .main
        ) { [weak self] _ in
            self?.loadMedications() // Directly calling the logic here
        }
    }

    deinit {
        // Unregister block-based observers
        if let observer = didBecomeActiveObserver {
            NotificationCenter.default.removeObserver(observer)
        }
        if let logObserver = medicationLoggedObserver {
            NotificationCenter.default.removeObserver(logObserver)
        }
    }

    // This is now a standard Swift private function with no @objc
...
extension MainMedicationViewController {
    private func updateDose(_ dose: TodayDoseItem, status: DoseLogStatus) {
        let log = DoseLog(id: UUID(), medicationID: dose.medicationID, doseID: dose.id, scheduledTime: dose.scheduledTime, loggedAt: Date(), status: DoseStatus(from: status), day: Date().startOfDay)
