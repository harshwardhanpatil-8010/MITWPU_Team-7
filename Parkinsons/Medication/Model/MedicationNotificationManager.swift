...
//
// Architecture:
//   • Medications at the SAME scheduled time share ONE notification (grouped).
//   • Each group notification carries a "payloads" array in userInfo so the
//     alarm screen can cycle through all due meds at once.
//   • Each dose still has its own follow-up notification (15 min after on-time).
//   • cancelNotifications / cancelOnTimeNotification / scheduleSkipFollowUp
//     work precisely — they do NOT call rescheduleAll() (which would rebuild
//     every notification from scratch and is too slow/disruptive mid-session).
...
final class MedicationNotificationManager {
    ...
    private let followUpDelay: TimeInterval = 15 * 60   // 15 minutes
    private let iso = ISO8601DateFormatter()

    // MARK: - Group notification identifier helpers
    //
    // On-time group:   "group_<ISO-date-string>"          e.g. group_2026-05-10T08:00:00Z
    // Follow-up group: "group_<ISO-date-string>_followup"
    // Per-dose follow-up (for skip reschedule): "<doseID>_followup"
    ...
    // Full rebuild — call when medications are added / edited / deleted.
    func rescheduleAll() {
        scheduleAll()
    }
    ...
    private func scheduleAll() {
        ...
        context.perform { [weak self] in
            ...
            // --- Group doses by their normalised fire-date ---
            // Key: the Date rounded to minute precision (same minute = same group)
            ...
            for med in medications {
                ...
                for dose in doseSet {
                    ...
                    // On-time: only if not yet taken/skipped and still in the future
                    ...
                    // Follow-up: only if not yet taken and still in the future
    ...
    //
    // Because multiple doses share one group notification we cannot simply remove it
    // when ONE dose is logged — other doses in the group may still be pending.
    // Strategy: rebuild the group notification minus the logged dose.
    // This is done asynchronously on a background context so it's non-blocking.

    /// Call when a dose is marked TAKEN — cancel both on-time and follow-up for this dose.
    func cancelNotifications(forDoseID doseID: UUID) {
        rebuildGroupsExcluding(doseID: doseID, cancelFollowUp: true)
        // Also remove any per-dose follow-up scheduled by scheduleSkipFollowUp
    ...
    /// Call when a dose is marked SKIPPED — only cancel its on-time notification.
    /// The follow-up should still fire (or a new one will be scheduled by scheduleSkipFollowUp).
    func cancelOnTimeNotification(forDoseID doseID: UUID) {
        rebuildGroupsExcluding(doseID: doseID, cancelFollowUp: false)
    }

    /// Schedule a per-dose follow-up 15 min from NOW (used after user taps Skip).
    /// This is a SINGLE-dose notification, not a group, so it can be cancelled precisely.
    func scheduleSkipFollowUp(payload: MedicationAlarmPayload) {
        ...
        // Single-payload userInfo so AlarmVC/AppDelegate parse correctly
        ...
        // Use per-dose ID so it can be cancelled precisely later
        UNUserNotificationCenter.current().add(
    ...
    /// Fetch pending notifications, remove the group that contains doseID,
    /// then re-add it without that dose.
