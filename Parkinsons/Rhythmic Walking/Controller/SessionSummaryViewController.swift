...
//        }
//        
//        // Fix endDate FIRST, then reassign session so the fixed version is used below
//        if session.endDate == nil {
//            session.endDate = session.startDate.addingTimeInterval(TimeInterval(session.elapsedSeconds))
//            sessionData = session
//        }
//        
...
//        
//        // ✅ Use sessionData (the fixed version) not the original session
...
//            self.stepsTaken.text        = summary.steps > 0 ? "\(summary.steps)" : "No data"
...
//  1. On load, immediately populate labels from Core Data cached values (no wait).
//  2. Fire HealthKit fetch in parallel — when it returns, update labels with fresh data.
//  This means the user always sees something immediately, and values update if
//  HealthKit returns different/better data.
...
class SessionSummaryViewController: UIViewController {
    ...
    @objc private func backTapped() {
        // Pop exactly one level → lands on SetGoalVC (the VC that pushed this)
    ...
    func loadData() {
        ...
        // ── Step 1: Show cached Core Data values IMMEDIATELY ──────────────
        if let cached = DataStore.shared.cachedSummary(for: fixedSession) {
            apply(summary: cached)
        } else {
            // No cached data yet — show dashes
            setPlaceholders()
        }

        // Fetch previous session for change% (synchronous Core Data read — fast)
        let previousManaged = DataStore.shared.previousSession(before: fixedSession)

        // ── Step 2: Fire HealthKit fetch — update when it returns ──────────
        HealthKitManagerRhythmic.shared.fetchFullSummary(for: fixedSession) { [weak self] summary in
            guard let self else { return }
            // Only update if HealthKit returned meaningful data
            if summary.steps > 0 || summary.distanceMeters > 0 {
                ...
                // HealthKit returned nothing new — keep cached and compute change%
                self.applyChangePercents(summary: cached, previous: previousManaged)
            }
        }
    }

    // MARK: - Apply summary to UI
    ...
    // MARK: - Change % vs previous session
