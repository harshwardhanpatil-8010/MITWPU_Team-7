// StillSphereLandingViewController.swift
// Parkinsons

import UIKit

class StillSphereLandingViewController: UIViewController {
    ...
    private func setupMonth() {
        ...
        // Adjust for Monday start (firstWeekday = 2)
    ...
    private func loadProgress() {
        ...
        let completedDates = summaries.compactMap { summary -> Date? in
            guard let d = summary.date else { return nil }
    ...
    private func startGame(with date: Date) {
        let storyboard = UIStoryboard(name: "StillSphere", bundle: nil)
        if let gameVC = storyboard.instantiateViewController(withIdentifier: "StillSphereGameViewController") as? StillSphereGameViewController {
...
extension StillSphereLandingViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
